...
valid_label_dataset = pickle.load(open('./valid_label_dataset','rb'))
...
def get_batch(loc, batch_size, mode):
  ...
  batch_ehr_mask = []
  for each_id in range(len(batch_id)):
    print(ehr[each_id][0][1])
    print(type(ehr[each_id][0][1]))
    
    print(diag_dataset.keys())
    print(prod_dataset.keys())
    print(proc_dataset.keys())
    print("batch_ehr")
    tmp_ehr = np.zeros((max_ts, 12, word_dim))
    tmp_mask = np.zeros(max_ts)
    #print(tmp)
    
    tmp = [np.vstack((diag_dataset[ehr[each_id][0][0]],prod_dataset[int(ehr[each_id][0][1])],proc_dataset[ehr[each_id][0][2]]))]
    print(tmp)
    for j in range(1, len(ehr[each_id])):
      print(diag_dataset[ehr[each_id][j][0]])
      print(prod_dataset[int(ehr[each_id][j][1])])        
      print(proc_dataset[int(float(ehr[each_id][j][2]))])
      tmp.append(np.vstack((diag_dataset[ehr[each_id][j][0]],prod_dataset[int(ehr[each_id][j][1])],proc_dataset[int(float(ehr[each_id][j][2]))])))
    tmp = np.array(tmp)
...
for each_epoch in range(epoch):
  for iteration in range(0, len(train_label_dataset), batch_size):
    ...
    optimizer.zero_grad()
       ...
       # loss, similarity, pred, attention, response, query
    loss, s_loss, pred, att,response,query = model.get_loss(batch_criteria, batch_criteria_mask, 
    ...
    if iteration % (400*batch_size) == 0:
      ...
      with torch.no_grad():
        ...
        for valid_iter in range(0, len(valid_label_dataset), batch_size):
          ...
          val_loss, val_s_loss, val_pred, att,val_response,val_query = model.get_loss(batch_criteria, batch_criteria_mask, 
             batch_ehr, batch_ehr_mask, batch_demo, batch_label,
             query_network, memory_network, embedding_network, device)
